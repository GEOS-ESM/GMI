...
!
!
!-----------------NEW for FJX74 parameters for cloud grid now here------
      integer, parameter :: &
!.orig      LPAR= 57 , LWEPAR=34    &   !this can be set by CTM code
            LPAR=132 , LWEPAR=LPAR  &   !this can be set by CTM code
...
!.orig     ,JVL_=LPAR               &   ! vertical(levels) dim for J-values sent to CTM
           ,JVN_=101                &   ! max no. of J-values
           ,AN_=25                      ! # FJX aerosols in layer (needs NDX for each)
!.new      ,AN_=56                      ! # FJX aerosols in layer (needs NDX for each)
!
!-----------------------------------------------------------------------
!
!New parameters for GMI
      integer :: JINDO3, JINDO1D, JINDAceta, JINDAcetb
      real*8,  parameter :: SZAMAX=98.0d0
!
      ! JXL_: vertical(levels) dim for J-values computed within fast-JX
      integer, parameter ::  JXL_=LPAR+1, JXL1_=JXL_+1
...
!.sds  added MAPL physical constants being used
      real*8, parameter  :: MASFAC = 100.d0*(MAPL_AVOGAD/1000.0)/(MAPL_AIRMW*MAPL_GRAV*10.d0)
      ! ZZHT: scale height (cm) used above top of CTM ZHL(LPAR+1)
      real*8, parameter  :: ZZHT = 5.d5
      ! RAD: Radius of Earth (cm)
!.sds  added MAPL physical constants being used
      real*8, parameter  :: RAD = 100.0*MAPL_RADIUS
...
!
!-----------------------------------------------------------------------
      ! variables used to map fast-JX Js onto CTM Js
!-----------------------------------------------------------------------
...
!
      character*6 JVMAP(JVN_) !label of J-value used to match w/FJX Js
